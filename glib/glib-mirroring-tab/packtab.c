...
static long _pow[22], cluster, cmpcluster;
...
)
{
  ...
  for (i = 21; max < _pow[i]; i--)
    ;
  ii = i;
  while (i && !((min ^ max) & _pow[i]))
    i--;

  if (ii == i)
    {
      /* min is less than half of max */
      for (i = 21 - 1; min < _pow[i]; i--)
	;
      i++;
      return _pow[i];
    }
...
)
{
  ...
  for (i = 1 - t[lev] % 2; i <= nn + (t[lev] >> nn) % 2; i++)
    {
      nn -= (p[lev] = i);
      clusters[lev] = cluster = (i && nn >= 0) ? _pow[i] : t[lev];
  ...
  for (ii = 0; ii < k; ii++)
    {
      ...
      fprintf (f, "\n#define %sLev%d_%0*lX 0x%0X", table_name,
	       best_lev - lev - 1, digits, x[i] * _pow[n - nn], ofs);
      kk = x[i] * cluster;
      if (!lev)
	if (name)
	  for (j = 0; j < cluster; j++)
	...
	for (j = 0; j < cluster; j++, kk++)
	  fprintf (f, "\n  %sLev%d_%0*lX,  /* %0*lX..%0*lX */", table_name,
		   best_lev - lev, digits,
		   tab[lev][kk] * _pow[n - nn - best_p[lev]], digits,
		   x[i] * _pow[n - nn] + j * _pow[n - nn - best_p[lev]], digits,
		   x[i] * _pow[n - nn] + (j + 1) * _pow[n - nn - best_p[lev]] -
		   1);
      ofs += cluster;
      jj = i;
      for (j = 0; j < k; j++)
...
)
{
  ...
  for (i = best_lev - 1; i >= 0; i--)
	...
	fprintf (f, " >> %d", j);
      if (i)
	fprintf (f, " & 0x%02lx) +", _pow[best_p[best_lev - 1 - i]] - 1);
